...
/**
 * @author kailash
 * Java 8 is bringing on new Streams API java.util.stream package, which allow you to process elements of 
 * Java Collections in parallel. Java is inheritably sequential and there is no direct mean to introduce 
 * parallel processing at library level, stream API is going to fill that gap. By using this, you can 
 * filter elements of collection on given criterion e.g. if you have a List of Person, you can filter 
 * orders Person upon there states/properties.
 */
...
public class StreamEx {
	...
	public static void main(String[] args){
		...
		List<String> personDistinct = Person.getPersonList().stream().map((s)->s.getDesignation()).distinct().collect(Collectors.toList());
		System.out.println("person : "+personDistinct);
		
		//Minimum salary from stream
		Person personMinSalary = Person.getPersonList().stream().min((p1, p2) -> Integer.compare(p1.getSalary(), p2.getSalary())).get();
		System.out.println("person : Min Salary -> "+personMinSalary+"="+personMinSalary.getSalary());
		
		//Maximum salary from stream
		Person personMaxSalary = Person.getPersonList().stream().max((p1, p2) -> Integer.compare(p1.getSalary(), p2.getSalary())).get();
